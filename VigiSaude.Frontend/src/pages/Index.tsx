      ...
      <div className="flex-1 flex flex-col">
        <header className="h-14 flex items-center border-b bg-card px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">- {selectedProfile === "admin" ? "Administrador" : "Usuário Padrão"}</span>
          </div>
        ...
        <main className="flex-1 overflow-auto">{selectedProfile === "admin" ? <AdminDashboard /> : <UserDashboard />}</main>
      </div>
    </div>
  );
};
